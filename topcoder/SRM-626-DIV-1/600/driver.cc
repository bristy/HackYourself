...
int	charges;
		...
		__in >> __expected >> N >> from >> to >> weight >> charges;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << from << ", " << to << ", " << weight << ", " << charges << std::endl;
		std::cout.flush();

		__timer_start();

		NegativeGraphDiv1 __object;
		long long __actual = __object.findMin(N, from, to, weight, charges);
