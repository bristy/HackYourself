...
int	goal;
		...
		__in >> __expected >> numbers >> goal;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << numbers << ", " << goal << std::endl;
		std::cout.flush();

		__timer_start();

		ORSolitaire __object;
		int __actual = __object.getMinimum(numbers, goal);
