...
#include "PalindromeMatrixDiv2.cc"
...
int	columnCount;
		...
		__in >> __expected >> A >> rowCount >> columnCount;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << rowCount << ", " << columnCount << std::endl;
		std::cout.flush();

		__timer_start();

		PalindromeMatrixDiv2 __object;
		int __actual = __object.minChange(A, rowCount, columnCount);
