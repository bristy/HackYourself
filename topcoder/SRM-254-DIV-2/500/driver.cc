...
string	phrase;
		...
		__in >> __expected >> typed >> phrase;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << typed << ", " << phrase << std::endl;
		std::cout.flush();

		__timer_start();

		ListeningIn __object;
		string __actual = __object.probableMatch(typed, phrase);
