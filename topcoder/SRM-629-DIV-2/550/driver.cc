...
vector <int>	desiredWeight;
		...
		__in >> __expected >> containerVolume >> desiredWeight;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << containerVolume << ", " << desiredWeight << std::endl;
		std::cout.flush();

		__timer_start();

		CandyMaking __object;
		double __actual = __object.findSuitableDensity(containerVolume, desiredWeight);
